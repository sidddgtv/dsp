...
<div class=" text-end">
        ...
        <!-- Button for publish -->
        <button type="button" class="btn btn-success btn-sm" onclick="loadsummarymodal()">
			<i class="las la-file"></i> Summary
		</button>
...
this.parseExcel = function(file, type) {
  ...
  reader.onload = function(e) {
	...
	workbook.SheetNames.forEach(function(sheetName) {
        ...
        if(type == 1){
            //preview
            $("#show_tmp").html('<img src="<?php echo base_url('storage/uploads/loadingnew.gif');?>" alt="Loading" height="20" />');
            //send to PHP
            $.ajax({
                url: '<?php echo admin_url('schedule/preview'); ?>',
                type: 'post',
                data: { 'ls_headers': json_object_header, 'ls_values': json_object },
                success: function(response){
                    //alert(response);
                    var data = JSON.parse(response);
                    if(data.status){
                        $("#show_tmp").html(data.message);
                        //enable upload button
                        $('#week_btn').prop('disabled', false);
                    }else{
                        $("#show_tmp").html(data.message);
                        //disable upload button
                        $('#week_btn').prop('disabled', true);
                    }
...
function loadpublishmodal(){
	...
	$('#publish_message').html('Publish all schedules for Week: <strong>'+week+'</strong> | Year: <strong>'+year+'</strong>?');
...
function loadsummarymodal(){
	var year = $('#year_number').val();
	var week = $('#week_number').val();
	$('#summary_message').html('Summary for Week: <strong>'+week+'</strong> | Year: <strong>'+year+'</strong>');
    $('#summary_success_message').html('');
    $.ajax({
        type: 'POST',
        url: "<?php echo admin_url() . 'schedule/showsummarymodalbody'; ?>",
        data: {'week': week, 'year': year},
        dataType:'html',
        error: function() {
        },
        success: function(data) {
			$('#summary_success_message').html(data);
        }
    });
...
function publishschedule(){
    ...
    $.ajax({
        type: 'POST',
        url: "<?php echo admin_url() . 'schedule/publishschedule'; ?>",
        data: {'week': week, 'year': year},
        dataType:'html',
        error: function() {
        },
        success: function(data) {
			$('#publish_success_message').html('<i class="las la-check-circle"></i> Schedule published');
        }
    });
